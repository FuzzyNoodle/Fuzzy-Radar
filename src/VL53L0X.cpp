...
/*
This Pololu is modified by georgychen;
- Added some regAddr values
- Added setGPIO(bool true=high, false=low) function to set the GPIO output to high(pulled-up) or low.
*/
...
void VL53L0X::setGPIO(bool value)
{
	//writeReg(SYSTEM_INTERRUPT_CONFIG_GPIO, 0x04);
	if (value == false)
	{
		//setting GPIO pin to low
		writeReg(VL53L0X_REG_GPIO_HV_MUX_ACTIVE_HIGH, GPIO_OUTPUT_DRIVE_LOW);
	}
	else
	{
		//setting GPIO pin to high
		writeReg(VL53L0X_REG_GPIO_HV_MUX_ACTIVE_HIGH, GPIO_OUTPUT_DRIVE_HIGH);
	}
}
